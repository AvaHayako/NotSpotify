...
class Subscriber:
    ...
    # follow input artist
    def follow(self, aID):
        self.c.execute("insert into Follow values (%d,%d);"% (self.sub_ID, aID))
        self.db.commit()
    ...
    def unfollow(self, aID):
        self.c.execute("delete from Follow where subID=%d and aID=%d;"% (self.sub_ID, aID))
        self.db.commit()
        self.c.execute("Select aName from Artist where aID=%d"%(aID))
        a = self.c.fetchall()[0][0]
    ...
    # list all playlists
    def playlists(self):
        self.c.execute("select P.pName from Playlist P where P.subID=%d;"% (self.sub_ID))
        results = self.c.fetchall()
    ...
    def add_playlist(self, pName):
        self.c.execute("insert into Playlist values('%s','%s','%s',False);"% (str(self.sub_ID)+(pName.strip()), self.sub_ID, pName ))
        self.db.commit()
    ...
    def remove_playlist(self, pID):
        ...
        self.db.commit()
        print(f"= Removed Playlist {pName} =")
